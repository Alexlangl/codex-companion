...
type PriceDraft = ModelPriceSettings & {
...
export function PricingEditor({
  ...
  const [snapshot, setSnapshot] = useState<PricingSettingsSnapshot | null>(
    null,
  );
  ...
  function saveModel(): void {
    ...
    const aliases = aliasesText
      ...
      .filter(Boolean);
    const remaining = snapshot.overrides.models.filter(
      (item) => item.model !== originalModel,
    );
    const names = [
      ...remaining.flatMap((item) => [item.model, ...item.aliases]),
      model,
      ...aliases,
    ].map(normalizeName);
    if (
    ...
    void persist(
      { ...snapshot.overrides, models: [...remaining, next] },
      `已保存 ${model} 的价格。`,
    );
  }

  function editModel(
    price: ModelPriceSettings,
    originalModel: string | null,
  ): void {
    setError(null);
    setMessage("");
    setDraft({
      ...price,
      originalModel,
      aliasesText: price.aliases.join("\n"),
    });
  }
  ...
  function removeModel(price: ModelPriceSettings, builtin: boolean): void {
    if (
      ...
      !window.confirm(
        builtin
          ? `恢复 ${price.model} 的内置价格？`
          : `删除 ${price.model} 的自定义价格？`,
      )
    )
      return;
    void persist(
      {
        ...snapshot.overrides,
        models: snapshot.overrides.models.filter(
          (item) => item.model !== price.model,
        ),
      },
      builtin ? "已恢复内置价格。" : "已删除自定义价格。",
    );
  }
  ...
  const customNames = new Set(
    customModels
      .flatMap((model) => [model.model, ...model.aliases])
      .map(normalizeName),
  );
  const builtins = snapshot?.builtinModels ?? [];
  const builtinNames = new Set(
    builtins.map((model) => normalizeName(model.model)),
  );
  const rows = [
    ...customModels,
    ...builtins.filter((price) => !customNames.has(normalizeName(price.model))),
  ];
  const visibleRows = rows.filter((price) =>
    [price.model, ...price.aliases].some((name) =>
      name.toLowerCase().includes(search.trim().toLowerCase()),
    ),
  );
  const providerIds = [
    ...new Set([...Object.keys(providers), ...Object.keys(multipliers)]),
  ];

  return (
    <div className="pricing-editor">
      <p className="field-hint">
        单位：美元 / 百万 Token。支持添加 GPT-6
        等模型及别名；请按所用渠道填写价格。保存后更新历史成本估算，不影响实际账单。
      </p>
      {pricingPreview ? (
        <p className="warning-box">
          浏览器预览：此处保存仅用于界面演示，不修改桌面应用定价或统计。
        </p>
      ) : null}
      <div className="actions" ref={addButtonRef}>
        <Button
          disabled={!snapshot || busy || Boolean(draft)}
          onClick={addModel}
        >
          添加模型价格
        </Button>
      ...
      <div className="configuration-feedback" aria-live="polite">
        ...
        {error ? (
          <p className="error-banner" role="alert" id="pricing-error">
            {error}
          </p>
        ) : null}
      </div>
              ...
              return (
                <tr key={price.model}>
                  <th scope="row">
                    ...
                    {price.aliases.length ? (
                      <small>别名：{price.aliases.join("、")}</small>
                    ) : null}
                  </th>
                  <td>{price.inputPerMillion}</td>
                  <td>{price.cachedInputPerMillion}</td>
                  <td>{price.cacheWriteInputPerMillion}</td>
                  ...
                  <td>
                    <div className="actions">
                      <Button
                        ...
                        variant="secondary"
                        label={`编辑 ${price.model} 价格`}
                        onClick={() =>
                          editModel(price, custom ? price.model : null)
                        }
                      >
                        编辑
                      </Button>
                      {custom ? (
                        <Button
                          disabled={busy || Boolean(draft)}
                          variant="ghost"
                          label={`${builtin ? "恢复默认" : "删除"} ${price.model}`}
                          onClick={() => removeModel(price, builtin)}
                        >
                          {builtin ? "恢复默认" : "删除"}
                        </Button>
                      ) : null}
      ...
      <details className="pricing-multipliers">
        ...
        <form
          onSubmit={(event) => {
            event.preventDefault();
            if (!snapshot) return;
            const providerMultipliers = Object.fromEntries(
              Object.entries(multipliers).filter(([, value]) => value.trim()),
            );
            if (
              Object.values(providerMultipliers).some(
                (value) =>
            ...
            void persist(
              { ...snapshot.overrides, providerMultipliers },
              "已保存账号成本倍率。",
            );
          }}
        >
          <fieldset
            className="configuration-stack"
            disabled={busy || !snapshot || Boolean(draft)}
          >
            ...
            <div className="form-grid">
              {providerIds.map((id) => (
                <Field
                  ...
                  label={
                    providers[id]
                      ? providerAccountTitle(providers[id])
                      : `历史账号 ${id}`
                  }
                >
                  <input
            ...
            <p className="field-hint" id="multiplier-help">
              倍率必须大于 0，留空恢复默认。
            </p>
